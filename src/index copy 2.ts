...
class BonePhysics {
  ...
  applyRandomForce() {
    if (!this.settings.FixPosition && this.settings.Mass > 0) {
      ...
      this.velocity.x += (Math.cos(angle) * force) / this.settings.Mass;
      this.velocity.y += (Math.sin(angle) * force) / this.settings.Mass;
    }
  }
  ...
  private updateLookAtChild(deltaTime: number) {
    if (
    ...
    try {
      ...
      let targetAngleDegrees = targetAngleRad * (180 / Math.PI);
      const currentAngle = this.normalizeAngle(this.currentRotation);
      targetAngleDegrees = this.normalizeAngle(targetAngleDegrees);

      let angleDiff = targetAngleDegrees - currentAngle;
      if (angleDiff > 180) angleDiff -= 360;
      if (angleDiff < -180) angleDiff += 360;

      if (this.settings.AngleLimit < 180) {
        const angleLimit = this.settings.AngleLimit;
        if (Math.abs(angleDiff) > angleLimit) {
          angleDiff = Math.sign(angleDiff) * angleLimit;
        }
        targetAngleDegrees = currentAngle + angleDiff;
      } else {
        targetAngleDegrees = currentAngle + angleDiff;
      }
      ...
      const angleSpringForce =
        (targetAngleDegrees - this.currentRotation) *
      ...
      const maxForce = 500;
      totalAngleForce = Math.max(
        -maxForce,
        Math.min(maxForce, totalAngleForce)
      );

      this.angleVelocity += (totalAngleForce / this.settings.Mass) * deltaTime;

      const maxAngularVelocity = 90 + Math.abs(angleDiff) * 2;
      if (Math.abs(this.angleVelocity) > maxAngularVelocity) {
        this.angleVelocity = Math.sign(this.angleVelocity) * maxAngularVelocity;
      }

      this.currentRotation += this.angleVelocity * deltaTime;
      this.currentRotation = this.normalizeAngle(this.currentRotation);
    } catch (error) {
      console.warn("Error in LookAtChild calculation:", error);
    }
  }

  private updateRotationWithRK4(deltaTime: number, targetRotation: number) {
    const calculateDerivatives = (rot: number, aVel: number) => {
      const angleSpringForce =
        (targetRotation - rot) * this.settings.SupportSpringShiftiness;
      const angleDampingForce = -aVel * this.settings.SupportSpringDamping;
      ...
      return {
        aAcc: totalAngleForce / this.settings.Mass,
        aVel: aVel,
      };
    };

    const k1 = calculateDerivatives(this.currentRotation, this.angleVelocity);
    const k2 = calculateDerivatives(
      this.currentRotation + k1.aVel * deltaTime * 0.5,
      this.angleVelocity + k1.aAcc * deltaTime * 0.5
    );
    const k3 = calculateDerivatives(
      this.currentRotation + k2.aVel * deltaTime * 0.5,
      this.angleVelocity + k2.aAcc * deltaTime * 0.5
    );
    const k4 = calculateDerivatives(
      this.currentRotation + k3.aVel * deltaTime,
      this.angleVelocity + k3.aAcc * deltaTime
    );

    this.currentRotation +=
      ((k1.aVel + 2 * k2.aVel + 2 * k3.aVel + k4.aVel) * deltaTime) / 6;
    this.angleVelocity +=
      ((k1.aAcc + 2 * k2.aAcc + 2 * k3.aAcc + k4.aAcc) * deltaTime) / 6;
  }

  private updateWithEuler(
    deltaTime: number,
    targetX: number,
    targetY: number,
    ...
    if (this.settings.LookAtChild) {
      this.updateLookAtChild(deltaTime);
    } else {
      const angleSpringForce =
    ...
    const maxForceBasedOnDistance = 500;
    const distance = Math.sqrt(
      Math.pow(targetX - this.currentPosition.x, 2) +
        Math.pow(targetY - this.currentPosition.y, 2)
    );
    const distanceForceLimit = distance * 100;

    totalForceX = Math.max(
      -Math.min(maxForceBasedOnDistance, distanceForceLimit),
      Math.min(
        Math.min(maxForceBasedOnDistance, distanceForceLimit),
        totalForceX
      )
    );
    totalForceY = Math.max(
      -Math.min(maxForceBasedOnDistance, distanceForceLimit),
      Math.min(
        Math.min(maxForceBasedOnDistance, distanceForceLimit),
        totalForceY
  ...
  update(deltaTime: number, holderPosition: PIXI.Point) {
    ...
    this.updateWithEuler(deltaTime, targetX, targetY, targetRotation);
...
async function init() {
  ...
  if (physicsConfig.VersionCode > 3) {
    console.warn("Конфигурация новой версии - возможна несовместимость");
  }
  ...
  for (const bone of spineboy.skeleton.bones) {
    if (physicsBoneNames.includes(bone.data.name)) {
      ...
      physicsBones.push(new BonePhysics(bone, settings, physicsConfig));
